...
export class InsightsParser {
  ...
  private static extractInsightsAndRoast(parsed: any): { insights: any; roast?: string } {
  ...
  private static validateInsights(insights: any): { insights: ParsedInsights; errors: string[] } {
    ...
    const hasExpectedInsights = this.EXPECTED_INSIGHT_KEYS.some(
      expectedKey => Object.keys(validatedInsights).some(
        actualKey => actualKey.toLowerCase().includes(expectedKey.toLowerCase().split(' ')[0])
      )
    );
  ...
  private static fallbackParsing(rawData: string): ParsedInsights {
    ...
    if (rawData.toLowerCase().includes('portfolio') || 
      ...
      const truncatedText = rawData.length > 500 ? rawData.substring(0, 500) + "..." : rawData;
